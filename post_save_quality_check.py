...
def process_quality_after_save(table_name: str = "all", limit: int = 10) -> Dict[str, int]:
    """
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"Noticias con resumen vacío: {quality_stats['empty_news_summaries']}")
        logger.info(f"Registros de sentimiento con análisis vacío: {quality_stats['empty_sentiment_analysis']}")
        logger.info(f"Señales de trading con análisis experto vacío: {quality_stats['empty_trading_signals_analysis']}")
        logger.info("=" * 80)
        ...
        if table_name in ["news", "all"] and quality_stats['empty_news_summaries'] > 0:
            ...
            logger.info(f"Se procesaron {result['news_processed']} noticias con resumen vacío")
        
        if table_name in ["sentiment", "all"] and quality_stats['empty_sentiment_analysis'] > 0:
            # Procesar registros de sentimiento con análisis vacío
            result["sentiment_processed"] = process_empty_sentiment_analysis(connection, limit)
            logger.info(
                f"Se procesaron {result['sentiment_processed']} registros de sentimiento con análisis vacío"
            )
        
        if table_name in ["signals", "all"] and quality_stats['empty_trading_signals_analysis'] > 0:
            # Procesar señales de trading con análisis experto vacío
            result["signals_processed"] = process_empty_trading_signals_analysis(connection, limit)
            logger.info(
        ...
        logger.info(f"Registros de sentimiento procesados: {result['sentiment_processed']}")
