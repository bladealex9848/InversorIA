...
class DataValidator:
    ...
    def validate_market_news(self, news_data):
        ...
        if not validated_data.get("title"):
            validated_data["title"] = f"Noticia financiera - {datetime.now().strftime('%Y-%m-%d')}"
        ...
        if not validated_data.get("symbol"):
            ...
            logger.warning("Se asignó SPY como símbolo por defecto para una noticia sin símbolo")
        ...
        if not validated_data.get("impact") or validated_data.get("impact") not in ["Alto", "Medio", "Bajo"]:
            validated_data["impact"] = "Medio"  # Valor por defecto
            logger.warning("Se asignó impacto Medio por defecto a una noticia sin impacto válido")
        ...
        if self.ai_expert and (not validated_data.get("summary") or len(validated_data.get("summary", "")) < 20):
            try:
                ...
                if generated_summary and len(generated_summary) > 30:
                    ...
                    logger.info(f"Resumen generado con IA para noticia: {title[:30]}...")
                else:
                    # Si no se pudo generar un resumen, crear uno genérico pero informativo
                    validated_data["summary"] = f"Noticia sobre {validated_data.get('symbol')} relacionada con {title[:50]}..."
                    logger.warning(f"No se pudo generar resumen con IA, usando genérico para: {title[:30]}...")
            except Exception as e:
                logger.error(f"Error generando resumen con IA: {str(e)}")
                # Asignar un resumen genérico en caso de error
                validated_data["summary"] = f"Información financiera sobre {validated_data.get('symbol')}"
        
        # Si el resumen existe pero está en inglés, traducirlo
        elif self.ai_expert and validated_data.get("summary") and self._is_english_text(validated_data.get("summary", "")):
            try:
                prompt = f"""
                ...
                if translated_summary and len(translated_summary) > 30:
                    ...
                    logger.info(f"Resumen traducido con IA para noticia: {validated_data.get('title')[:30]}...")
            except Exception as e:
    ...
    def validate_market_sentiment(self, sentiment_data):
        ...
        if not validated_data.get("overall") or validated_data.get("overall") not in ["Alcista", "Bajista", "Neutral"]:
            validated_data["overall"] = "Neutral"  # Valor por defecto
            logger.warning("Se asignó sentimiento Neutral por defecto a un registro sin sentimiento válido")
        
        # Asegurar que el campo de análisis esté presente y sea informativo
        if self.ai_expert and (not validated_data.get("analysis") or len(validated_data.get("analysis", "")) < 50):
            try:
                ...
                if generated_analysis and len(generated_analysis) > 100:
                    ...
                    validated_data["analysis"] = f"El mercado muestra un sentimiento {overall.lower()} con VIX en {vix} y tendencia {sp500_trend.lower()} en el S&P500."
                    logger.warning("No se pudo generar análisis con IA, usando genérico")
            except Exception as e:
                logger.error(f"Error generando análisis con IA: {str(e)}")
                # Asignar un análisis genérico en caso de error
                validated_data["analysis"] = f"Sentimiento de mercado: {validated_data.get('overall', 'Neutral')}"
        
        # Asegurar que los indicadores técnicos estén presentes
        if not validated_data.get("technical_indicators"):
            validated_data["technical_indicators"] = "No hay datos de indicadores técnicos disponibles"
