...
def validate_market_data(data: pd.DataFrame) -> bool:
    ...
    try:
        ...
        if not all(data['High'] >= data['Low']) or \
           not all(data['High'] >= data['Open']*0.99) or \
           not all(data['High'] >= data['Close']*0.99) or \
           not all(data['Low'] <= data['Open']*1.01) or \
           not all(data['Low'] <= data['Close']*1.01):
            return False
...
def _get_finnhub_data(symbol: str, resolution: str = 'D') -> pd.DataFrame:
    ...
    try:
        ...
        resolution_map = {'1d': 'D', '1h': '60', '30m': '30', '15m': '15', '5m': '5', '1m': '1'}
...
def fetch_market_data(symbol: str, period: str = '6mo', interval: str = '1d') -> pd.DataFrame:
    ...
    if not _data_cache.can_request(symbol):
        logger.info(f"Limitando solicitudes para {symbol}, retornando últimos datos conocidos o sintéticos")
    ...
    try:
        ...
        if not validate_market_data(data):
            logger.warning(f"Datos inválidos para {symbol} en yfinance, intentando fuentes alternativas")
            ...
            if data is None or not validate_market_data(data):
                ...
                if data is None or not validate_market_data(data):
                    ...
                    if data is None or not validate_market_data(data):
                        logger.warning(f"Todas las fuentes fallaron para {symbol}, generando datos sintéticos")
        ...
        self.strategies_catalog = {
            ...
            "bajistas": [
                ...
                },
                {
                    ...
                    "indicators": {
                        "required": ["Bearish_Channel", "RSI", "Stochastic", "MACD_Histogram"],
...
class TechnicalAnalyzer:
        ...
        try:
            ...
            for i in range(1, len(df)):
                ...
                if df['Low'].iloc[i] > df['High'].iloc[i-1]:
                    df['Gap'].iloc[i] = (df['Low'].iloc[i] / df['High'].iloc[i-1] - 1) * 100
                # Gap bajista
                elif df['High'].iloc[i] < df['Low'].iloc[i-1]:
                    df['Gap'].iloc[i] = (df['High'].iloc[i] / df['Low'].iloc[i-1] - 1) * 100
            ...
            for i in range(len(df)):
                ...
                if total_range > 0:  # Evitar división por cero
                    ...
                    if (lower_shadow > 2 * body_size and 
                        ...
                        if i > 20 and df['Close'].iloc[i-20:i].mean() < df['Close'].iloc[i]:
                            df['Hanger'].iloc[i] = True
                        # Es un Hammer si está en una tendencia bajista
                        elif i > 20 and df['Close'].iloc[i-20:i].mean() > df['Close'].iloc[i]:
        ...
        try:
            ...
            signals = {
                "trend": {
                    ...
                    "ema_trend": "alcista" if ('EMA_20' in df.columns and 'EMA_50' in df.columns and 
                                             current.get('EMA_20', 0) > current.get('EMA_50', 0)) else "bajista",
                    "sma_200": "por_encima" if ('Close' in df.columns and 'SMA_200' in df.columns and 
                                              current.get('Close', 0) > current.get('SMA_200', 0)) else "por_debajo"
                },
                "momentum": {
                    "rsi": float(current.get('RSI', 50)),
                    "rsi_condition": self._get_rsi_condition(current.get('RSI', 50)),
                    "rsi_trend": "alcista" if ('RSI' in df.columns and current.get('RSI', 50) > previous.get('RSI', 50)) else "bajista",
                    "stoch_k": float(current.get('Stoch_K', 50)),
                    "stoch_d": float(current.get('Stoch_D', 50)),
                    "stoch_trend": "alcista" if ('Stoch_K' in df.columns and 'Stoch_D' in df.columns and 
                                            current.get('Stoch_K', 0) > current.get('Stoch_D', 0)) else "bajista"
                },
                "volatility": {
                ...
                "volume": {
                    ...
                    "obv_trend": "alcista" if ('OBV' in df.columns and previous.get('OBV', 0) < current.get('OBV', 0)) else "bajista",
                    "vwap_position": "por_encima" if ('Close' in df.columns and 'VWAP' in df.columns and 
                                                    current.get('Close', 0) > current.get('VWAP', 0)) else "por_debajo"
                },
                "patterns": {
                    "gap": float(current.get('Gap', 0)),
                    "gap_direction": "alcista" if current.get('Gap', 0) > 0 else "bajista" if current.get('Gap', 0) < 0 else "ninguno",
    ...
    def _analyze_options_strategy(self, signals):
        ...
        try:
            ...
            direction = "CALL" if signals["overall"]["signal"] in ["compra", "compra_fuerte"] else "PUT"
            ...
            if signals["volatility"]["volatility_state"] == "alta":
                ...
                timeframe = "MEDIO-LARGO"  # Convergencia de tendencias -> plazo más largo
            ...
            if direction == "CALL":
                ...
                elif signals["trend"]["sma_20_50"] == "alcista" and signals["momentum"]["rsi_trend"] == "alcista":
    ...
    def analyze_multi_timeframe(self, symbol: str, timeframes: List[str] = ["1d", "1wk", "1mo"]) -> Dict:
            ...
            for tf in timeframes:
                try:
                    ...
                    if data is None or len(data) < 20:
                        logger.warning(f"Datos insuficientes para {symbol} en timeframe {tf}")
                        continue
                    ...
                    logger.error(f"Error analizando {symbol} en timeframe {tf}: {str(tf_error)}")
            ...
            if results:
                ...
                if score_buy > score_sell * 1.5:
                    consolidated = {
                        ...
                        "timeframe_alignment": "fuerte" if len(results) >= 2 else "moderada"
                    }
        ...
        try:
            ...
            for i in range(-5, 0):
                ...
                # Patrón: Martillo (alcista en tendencia bajista)
                    ...
                    if idx > 10 and df['Close'].iloc[idx-10:idx].mean() < open_price:
                        patterns.append({
                            "pattern": "Martillo",
                            "position": idx,
                            "date": df.index[idx],
                ...
                elif (body_percent > 80 and 
                      upper_shadow < 0.1 * body_size and 
                      lower_shadow < 0.1 * body_size):
                    
                    pattern_type = "bullish continuation" if is_bullish else "bearish continuation"
                    pattern_desc = "Fuerte presión compradora" if is_bullish else "Fuerte presión vendedora"
            ...
            if len(df) >= 3:
                ...
                last_two = df.iloc[-2:]
                if (last_two['Close'].iloc[0] < last_two['Open'].iloc[0] and  # Primera vela bajista
                    last_two['Close'].iloc[1] > last_two['Open'].iloc[1] and  # Segunda vela alcista
                    last_two['Close'].iloc[1] > last_two['Open'].iloc[0] and  # Envuelve el cuerpo
                    last_two['Open'].iloc[1] < last_two['Close'].iloc[0]):
                ...
                elif (last_two['Close'].iloc[0] > last_two['Open'].iloc[0] and  # Primera vela alcista
                      last_two['Close'].iloc[1] < last_two['Open'].iloc[1] and  # Segunda vela bajista
                      last_two['Close'].iloc[1] < last_two['Open'].iloc[0] and  # Envuelve el cuerpo
                      last_two['Open'].iloc[1] > last_two['Close'].iloc[0]):
    ...
    def get_support_resistance(self, data: pd.DataFrame = None, n_levels: int = 3) -> Dict:
        ...
        try:
            ...
            all_supports = sorted([l for l in lows + ma_levels + list(fib_levels.values()) if l < close])
            all_resistances = sorted([l for l in highs + ma_levels + list(fib_levels.values()) if l > close])
            
            # Agrupar niveles cercanos (dentro del 0.5%)
            grouped_supports = []
            for level in all_supports:
                if not grouped_supports or abs(level - grouped_supports[-1]) / level > 0.005:
                    grouped_supports.append(level)
            
            grouped_resistances = []
            for level in all_resistances:
                if not grouped_resistances or abs(level - grouped_resistances[-1]) / level > 0.005:
            ...
            resistances = sorted(grouped_resistances, key=lambda x: abs(close - x))[:n_levels]
