...
# Función para cargar configuración desde secrets.toml
    ...
    try:
        ...
        if os.path.exists(secrets_path):
            ...
            config = {
                ...
                "model": secrets.get("OPENAI_API_MODEL", "gpt-4.1-nano"),
                "assistant_id": secrets.get("ASSISTANT_ID", None),
            }
            return config
        else:
            logger.warning("No se encontró el archivo secrets.toml")
            return {"model": "gpt-4.1-nano", "assistant_id": None, "api_key": None}
    except Exception as e:
        logger.error(f"Error cargando configuración de OpenAI: {str(e)}")
        return {"model": "gpt-4.1-nano", "assistant_id": None, "api_key": None}


# Función para obtener análisis experto
def get_expert_analysis_from_scanner(prompt):
    """
    Obtiene análisis experto utilizando el asistente configurado en ASSISTANT_ID
    o la API directa como alternativa
    ...
    try:
        ...
        try:
            ...
            # Si no se puede importar ai_utils, usar la configuración local
            logger.info(
                "No se pudo importar ai_utils.get_expert_analysis, usando configuración local"
            )
            ...
            # Si tenemos assistant_id y api_key, usar la API de asistentes
                        ...
                        if message.role == "assistant":
                            ...
                            if hasattr(message, "content") and len(message.content) > 0:
                                ...
                                if hasattr(message_content, "text"):
                                    ...
                                    and "text" in message_content
                                ):
                                    return message_content["text"].get(
                                        "value", message_content["text"]
                                    )

                # Si no se pudo obtener la respuesta, usar la API directa
                logger.warning(
                    "No se pudo obtener respuesta del asistente, usando API directa"
                )

            # Si no tenemos assistant_id o api_key, o si falló el asistente, usar la API directa
            ...
            # Si no tenemos ni assistant_id ni api_key, generar respuesta simulada
            logger.warning(
                "No se pudo conectar con OpenAI, generando respuesta simulada"
            )
        ...
        logger.error(f"Error en get_expert_analysis_from_scanner: {str(e)}")
                ...
                if not sector_high_conf.empty:
                    ...
                    if selected_high_conf_asset:
                        ...
                        with analysis_tabs[5]:
                            ...
                            # Intentar cargar el cliente de OpenAI desde session_state
                            if "openai" in st.session_state and st.session_state.get(
                                "openai_configured", False
                            ):
                                openai_client = st.session_state.openai

                            # Intentar cargar el experto de IA desde session_state
                            if (
                                "ai_expert" in st.session_state
                                and st.session_state.get("ai_expert_configured", False)
                            ):
                                ai_expert = st.session_state.ai_expert
                            ...
                            if st.button(
                                ...
                                with st.spinner(
                                    ...
                                    try:
                                        ...
                                        # Usar la función get_expert_analysis_from_scanner que prioriza el asistente
                                        expert_analysis = (
                                            get_expert_analysis_from_scanner(prompt)
                                        )
