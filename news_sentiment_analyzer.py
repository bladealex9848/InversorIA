...
class NewsSentimentAnalyzer:
    ...
    def __init__(self, api_keys=None, openai_client=None):
        ...
        self.news_sources = [
            ...
            "MarketWatch", "Investing.com", "Yahoo Finance", "Seeking Alpha", "Barron's",
            "The Economist", "Morningstar", "Benzinga", "Zacks", "TheStreet"
        ]
        self.cache = {}
    ...
    def get_news_from_finnhub(self, symbol: str, days_back: int = 7) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"Error obteniendo noticias de Finnhub: {response.status_code}")
            return []
    ...
    def get_news_from_alpha_vantage(self, symbol: str, days_back: int = 7) -> List[Dict]:
        """
        ...
        try:
            ...
            logger.warning(f"Error obteniendo noticias de Alpha Vantage: {response.status_code}")
            return []
    ...
    def get_news_from_web_search(self, symbol: str, company_name: str = None) -> List[Dict]:
        """
            ...
            with DDGS() as ddgs:
                ...
                if results:
                    ...
                    for item in results:
                        ...
                        if any(word in title_lower for word in ["up", "rise", "gain", "bull", "positive", "growth"]):
                            sentiment_value = 0.7  # Positivo
                        elif any(word in title_lower for word in ["down", "fall", "drop", "bear", "negative", "loss"]):
                            sentiment_value = 0.3  # Negativo
                        
                        # Verificar si la fuente es confiable
                        source = item.get("source", "")
                        is_reliable = any(reliable_source.lower() in source.lower() 
    ...
    def analyze_sentiment_with_ai(self, news_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            news_text = ""
            for i, item in enumerate(news_data[:5]):  # Limitar a 5 noticias para el análisis
            ...
            response = self.openai_client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "Eres un analista financiero experto especializado en análisis de sentimiento de mercado."},
    ...
    def _analyze_sentiment_basic(self, news_data: List[Dict]) -> Dict:
        ...
        if len(news_data) - positive_mentions - negative_mentions > 0:
            key_factors.append(f"{len(news_data) - positive_mentions - negative_mentions} menciones neutrales")
    ...
    def get_consolidated_news_and_sentiment(self, symbol: str, company_name: str = None) -> Dict:
        """
        ...
        result = {
            ...
            "sources": list(set(item.get("source", "Desconocida") for item in all_news))
        }
